...
//         checked={checked}
//         onChange={onChange}
//         className={`relative inline-flex h-6 w-10 cursor-pointer rounded-full transition-colors
//                     ${checked ? "bg-primary" : "bg-gray-300"}`}
//     >
...
// endDate,
// setEndDate,
// favorites = [],          /* ← comes from App.jsx */
// onSearch,
// onSaveFavorite           /* ← called when “save to fav” is on */
// }) {
// /* basic search state */
// const [origin,            setOrigin]            = useState("");
// const [destination,       setDestination]       = useState("");
...
// const [minLoadedRPM,      setMinLoadedRPM]      = useState("");

// /* favorite switch */
// const [saveToFav,         setSaveToFav]         = useState(false);

// /* helper lives INSIDE component so it sees state vars */
// function itemTypeFlags() {
//     return {
//     is_hazardous:              hazardousOnly  ? true : null,
//     is_high_value:             highValueOnly  ? true : null,
//     is_temperature_controlled: tempControlOnly? true : null,
//     };
// }
...
//     /* add advanced filters only when section enabled */
//     if (filtersOn) {
//     Object.assign(base, {
//         capacity_type:   capacityType,
//         min_weight:      minWeight,
//         min_loaded_rpm:  minLoadedRPM,
//         ...itemTypeFlags()
//     });
//     }
...
//     /* persist as favourite if requested */
//     if (saveToFav && onSaveFavorite) {
//     onSaveFavorite({
//         origin:        base.origin,
//         destination:   base.destination,
//         capacity_type: base.capacity_type ?? "",
...
//                 </div>
//                 <div className="text-xs text-gray-600 mb-3">
//                 {fav.capacity_type || "Any"} • min wt&nbsp;
//                 {fav.min_weight || "—"} • min RPM&nbsp;
//                 {fav.min_loaded_rpm || "—"}
//                 </div>

//                 <button
//                 onClick={() => searchViaFavourite(fav)}
//                 className="w-full bg-primary hover:bg-primary-dark text-white text-sm font-semibold py-1.5 rounded-lg shadow"
//                 >
//                 Search
//                 </button>
...
//     <div className="bg-white bg-opacity-80 px-10 py-10 rounded-lg shadow-lg w-full max-w-3xl mt-14 mb-14">
...
//         {favorites.length > 0 && (
//         <div className="mb-6">
//             <h3 className="font-semibold mb-2">Quick Favorites</h3>
...
//                     setCapacityType(f.capacity_type ?? "");
//                     setMinWeight(f.min_weight ?? "");
//                     setMinLoadedRPM(f.min_loaded_rpm ?? "");
//                     setHazardousOnly(!!f.is_hazardous);
//                     setHighValueOnly(!!f.is_high_value);
//                     setTempControlOnly(!!f.is_temperature_controlled);
//                     setFiltersOn(true);
//                 }}
//                 className="whitespace-nowrap px-4 py-1 rounded-md border border-primary text-primary text-sm hover:bg-primary hover:text-white transition"
//                 >
//                 {f.origin} → {f.destination}
//                 </button>
//             ))}
...
//         <div className="flex items-center">
//             <CitySearchBox query={origin} setQuery={setOrigin} className="flex-grow"/>
//             <ToggleSwitch id="toggle-origin" checked={includeOrigin}
//                         onChange={() => setIncludeOrigin(x=>!x)} />
//         </div>
//         </div>
...
//         <div className="flex items-center">
//             <CitySearchBox query={destination} setQuery={setDestination}/>
//             <ToggleSwitch id="toggle-dest" checked={includeDestination}
//                         onChange={() => setIncludeDestination(x=>!x)} />
//         </div>
//         </div>
...
//         <div className="flex items-center mb-4">
//             <h3 className="font-semibold text-lg">Additional Filters</h3>
//             <ToggleSwitch id="toggle-filters" className="ml-auto"
//                         checked={filtersOn} onChange={()=>setFiltersOn(x=>!x)} />
//         </div>
...
//             <span className="font-medium mr-2">Item Type</span>
//             {[
//                 {label:"Hazardous",state:hazardousOnly,set:setHazardousOnly},
//                 {label:"High Value",state:highValueOnly,set:setHighValueOnly},
//                 {label:"Temperature Controlled",state:tempControlOnly,set:setTempControlOnly},
//             ].map(({label,state,set})=>(
//                 <button key={label} type="button" onClick={()=>set(x=>!x)}
//                         className={`px-4 py-1 rounded-md border text-sm transition
//                                     ${state ? "border-primary text-primary font-semibold"
//                                             : "border-gray-300 text-gray-500"}`}>
//                 {label}
//                 </button>
//             ))}
...
//             <div>
//                 <label className="font-medium block mb-1">Minimum Weight (lbs)</label>
//                 <input type="number" min="0" value={minWeight}
...
//             <div>
//                 <label className="font-medium block mb-1">Minimum Rate Per Mile</label>
//                 <input type="number" min="0" step="0.01" value={minLoadedRPM}
//                     onChange={e=>setMinLoadedRPM(e.target.value)}
//                     className="w-full border border-gray-300 rounded px-3 py-2"
//                     placeholder="Enter minimum loaded RPM"/>
//             </div>
//             </div>
//         </div>
//         </div>
...
//         <HeartIcon className={`w-6 h-6 mr-3 ${saveToFav?"text-primary":"text-black"}`}/>
//         <h3 className="font-semibold text-lg">Save to My Favorites</h3>
//         <ToggleSwitch id="toggle-fav" className="ml-auto"
//                         checked={saveToFav} onChange={()=>setSaveToFav(x=>!x)}/>
//         </div>
